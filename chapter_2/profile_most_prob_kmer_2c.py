...
def profile_most_probable_kmer(seq: str, k: int, profile_matrix: list[str]) -> str:
    profile_dict = dict(zip(['A', 'C', 'G', 'T'], 
        [[float(x) for x in profile_matrix[i].split(' ')] for i in range(0, len(profile_matrix))]
        ))
...
if __name__ == '__main__':
    with open('chapter_2/inputs/profile_most_probable_kmer.txt', 'r') as in_fh:
        ...
        seq = text[0].strip()
        k = int(text[1].strip())
        profile_matrix = text[2:]
        print(profile_most_probable_kmer(seq, k, profile_matrix))
